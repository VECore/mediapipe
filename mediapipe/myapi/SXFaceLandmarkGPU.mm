...
struct SXFaceLandmarkGPUGraph
{
    ...
    absl::Status startRun(void* sharedContext)
    {
        ...
        mGLContext = (__bridge mediapipe::PlatformGlContext)sharedContext;
        MP_RETURN_IF_ERROR(mGraph.StartRun({{SIDE_PACKET_NAME_FACE, mediapipe::MakePacket<int>(mMaxFaces)},
                                            {SIDE_PACKET_NAME_GPU_ORIGIN, mediapipe::MakePacket<int>(mBottomLeft ? 2 : 1)}}));
        return absl::OkStatus();
    }

    absl::Status process(CVPixelBufferRef pixel, int width, int height, SXColorFormat format)
    {
        auto frame_timestamp_us = std::chrono::steady_clock::now().time_since_epoch().count();
        mediapipe::Packet packet = mediapipe::MakePacket<mediapipe::GpuBuffer>(pixel);
        CVPixelBufferRelease(pixel);
        
        MP_RETURN_IF_ERROR(mGraph.AddPacketToInputStream(INPUT_STREAM, packet.At(mediapipe::Timestamp(frame_timestamp_us))));
